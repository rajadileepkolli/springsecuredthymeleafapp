...
public class SecurityServiceImpl implements SecurityService {
	...
	public User createUser(User user, boolean validated) {
		...
		user.getRoleList().parallelStream().filter(role -> role.getId() != null).collect(Collectors.toList())
				.forEach(role -> persistedRoles.add(getRoleById(role.getId())));
		/*
		 * if (roles != null) { for (Role role : roles) { if (role.getId() !=
		 * null) { persistedRoles.add(getRoleById(role.getId())); } } }
		 */
	...
	public User updateUser(User user, String id) throws SecuredAppException {
		...
		List<Role> updatedRoles = user.getRoleList().stream().filter(role -> role.getId() != null)
				.map(role -> getRoleById(role.getId())).collect(Collectors.toList());
		/*
	...
	public Role createRole(Role role) throws SecuredAppException {
		...
		List<Permission> permissions = role.getPermissions().stream().filter(permission -> permission.getId() != null)
				.map(permission -> findPermission(permission.getId())).collect(Collectors.toList());
		/*
	...
	public Role updateRole(Role role, String id) throws SecuredAppException {
		...
		List<Permission> updatedPermissions = role.getPermissions().parallelStream()
				.filter(permission -> permission.getId() != null).map(permission -> findPermission(permission.getId()))
