...
public class OrderServiceImpl implements OrderService {
	...
	public Order purchase(String email, Cart cart, String signature) {
		...
		OrderLines orderLines = (OrderLines) SerializationUtils
				.deserialize(SerializationUtils.serialize(cart.getOrderLines()));

		Order order = Order.builder().email(email).orderDate(LocalDate.now()).orderLines(orderLines).build();
