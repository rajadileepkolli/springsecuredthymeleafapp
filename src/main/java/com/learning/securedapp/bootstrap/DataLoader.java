...
public class DataLoader implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		if (categoryRepository.findAll().isEmpty()) {
			...
			categoryRepository.save(Arrays.asList(clothingCategory, foodCategory, mobileCategory));
		}

		if (productRepository.findAll().isEmpty()) {
			Product shirt = Product.builder().productName("Shirt").description("Spring Framework Guru Shirt")
					.price(Double.valueOf("18.95")).category(categoryRepository.findOne("1"))
					.imageUrl(
			...
			Product mug = Product.builder().productName("Mug").description("Spring Framework Guru Mug")
					.imageUrl(
					...
					.category(categoryRepository.findOne("1")).build();
		...
		if (roleRepository.findAll().isEmpty()) {
			...
			for (String string : roleList) {
				Role role = Role.builder().roleName(string)
						.permissions(Arrays.asList(permissionRepository.findByName("MANAGE_SETTINGS"))).build();
		...
		if (userRepository.findAll().isEmpty()) {
			User user = User.builder().userName("superadmin")
					.password(new BCryptPasswordEncoder(10).encode("superadmin")).email("rajadileepkolli@gmail.com")
					.enabled(true).roleList(roleRepository.findAll()).build();
